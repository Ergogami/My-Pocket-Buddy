...
export default function SearchPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-cyan-200 via-purple-100 via-pink-100 to-orange-200 pb-32">
      {/* Adventure Header */}
        ...
        <div className="mb-8">
          <div className="flex space-x-4 overflow-x-auto pb-4 snap-x">
            {programs.map((program) => {
              ...
              const borderColors = {
                1: 'border-orange-200', // Animal Adventures
                2: 'border-blue-200',   // Balance Masters  
                3: 'border-green-200'   // Strength Heroes
              };
              const dotColors = {
                1: 'bg-orange-400', // Animal Adventures
                2: 'bg-blue-400',   // Balance Masters
                3: 'bg-green-400'   // Strength Heroes
              };
              
              return (
                <div key={program.id} className="flex-shrink-0 w-72 snap-start">
                  <div className={`bg-white/90 backdrop-blur-md rounded-3xl border-4 ${borderColors[program.id]} overflow-hidden shadow-xl`}>
                    <div className={`bg-gradient-to-br ${program.color} p-4 text-white`}>
                      <div className="flex items-center space-x-3 mb-2">
                        <div className="w-10 h-10 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center">
                          <span className="text-xl">{program.icon}</span>
                        </div>
                        <h3 className="text-lg font-bubble">{program.title}</h3>
                      </div>
                      <p className="text-white/90 text-sm font-rounded font-bold">{program.description}</p>
                      <div className="mt-3 text-white/80 text-xs font-rounded font-bold">{programExercises.length} exercises</div>
                    </div>
                    <div className="p-3 space-y-2">
                      {programExercises.slice(0, 3).map((exercise) => (
                        <div key={exercise.id} className="flex items-center space-x-2 text-sm">
                          <span className={`w-2 h-2 ${dotColors[program.id]} rounded-full`}></span>
                          <span className="font-bubble text-gray-700">{exercise.name}</span>
                        </div>
                      ))}
                    </div>
                  </div>
